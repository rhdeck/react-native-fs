    ...
    private void upload(UploadParams params, UploadResult result) throws Exception {
        ...
        try {
            ...
            if (params.method.toLowerCase == "put" && params.files.size() == 1) {
                ...
                int buffer_size = (int) Math.ceil(totalFileLength / 100.f);
                ...
                while (fieldsIterator.hasNextKey()) {
                    ...
                    metaData += twoHyphens + boundary + crlf + "Content-Disposition: form-data; name=\"" + key + "\""
                ...
                for (ReadableMap map : params.files) {
                    ...
                    File file = new File(map.getString("filepath"));
                    String fileHeaderType = twoHyphens + boundary + crlf + "Content-Disposition: form-data; name=\""
                            + name + "\"; filename=\"" + filename + "\"" + crlf + "Content-Type: " + filetype + crlf;
                ...
                long requestLength = totalFileLength + stringData.length()
                        + params.files.toArray().length * crlf.length();
                connection.setRequestProperty("Content-length", "" + (int) requestLength);
                connection.setFixedLengthStreamingMode((int) requestLength);
                connection.connect();
